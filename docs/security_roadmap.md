...
| Retention / legal hold | 🟡 Per-row `legal_hold_reason` flag in migration 018 (triggers escalate immutability when set; FK cascade blocked). Admin API at `POST/DELETE/GET /api/admin/v1/legal-holds`. Runbooks: `docs/runbooks/legal_hold.md` (API as primary, direct SQL as break-glass), `docs/runbooks/tenant_purge.md` (enumerates held rows). Still open: per-table TTL config. | First SOC 2 audit | 🟢 Per-table TTL config + legal-hold flag + API + documented runbooks |
