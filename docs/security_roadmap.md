...
| Retention / legal hold | 🟡 Per-row `legal_hold_reason` flag landed in migration 018 — triggers escalate immutability when set (FK cascade also blocked). Tenant-purge runbook ([`docs/runbooks/tenant_purge.md`](runbooks/tenant_purge.md)) enumerates held rows; legal-hold apply / release runbook at [`docs/runbooks/legal_hold.md`](runbooks/legal_hold.md). Still open: per-table TTL config + an API endpoint wrapping the apply/release SQL. | First SOC 2 audit | 🟢 Per-table TTL config + legal-hold flag + API + documented runbooks |
