...
15. `018_legal_hold_flag.sql` — adds `legal_hold_reason text NULL` to `policy_audit_log` and `baseline_snapshots` + extends the trigger functions to escalate immutability when the flag is set. While a row is on legal hold (`legal_hold_reason IS NOT NULL`), every change except toggling the hold itself is rejected — including FK SET NULL cascade. Operator runbooks at `docs/runbooks/legal_hold.md` (apply / release) and `docs/runbooks/tenant_purge.md` (now enumerates held rows as a pre-condition).
